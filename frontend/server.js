...
app.prepare().then(() => {
  ...
  server.use('/medicines/new', (req, res) => {
    console.log(`Routing /medicines/new to ${targetHost}/medicines/new`);
    proxy.web(req, res, { target: `${targetHost}/medicines/new` });
  });

  server.post('/medicines', (req, res) => {
    console.log(`Routing /medicines post to ${targetHost}/medicines`);
    proxy.web(req, res, { target: `${targetHost}` });  // /medicinesをつけ足す必要はない
  });
  
  // 他のHTTPメソッドの/medicinesへのリクエストはNext.jsのハンドラーに渡す
  server.all('/medicines', (req, res) => {
    if (req.method !== 'POST') {
